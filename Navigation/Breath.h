...
public:
	...
	void initialize();
	void bfsTraversal(Vertex& v, Vertex& target, VertexList& path, ostream& out) {
		int** ppDistMtrx;
		int* pLeastCost;
		...
		while (num_selected < num_nodes)	 // ÀüºÎ´Ù ¼¿·ºÆ® µÇ¸é Å»Ãâ
		{
			...
			if (minID == -1) {
				out << "Error in FindShortestPath() ";
				out << "target is not connected to the start !!" << endl;
				break;
			}
			else {
			...
			for (int i = 0; i < num_nodes; i++)
			{
				...
				{
					out << "v" << i << setfill(' ') << setw(5) << "+oo" << ", ";
				}
				else
				{
					out << "v" << i << setfill(' ') << setw(5) << pLeastCost[i] << ", ";
				}

			}
			out << endl;
		}
		out << endl << " Previous Node in the shortest Paths  : " << endl;
		for (int i = 0; i < num_nodes; i++) {
			out << "  " << i << " <- " << pPrev[i];
		}
		out << endl << endl;
		for (int i = 0; i < num_nodes; i++) {
			out << " Shortest path from " << v.getID() << " to " << i << " : ";

			int k = i ;
			out << i << " <- ";
			while (pPrev[k] != v.getID()) {
				out << " " << pPrev[k] << " <- ";
				k = pPrev[k];
			}
			out << " " << v.getID();
...
public:	
	void initDistMtrx();
	void printDistMtrx();
	void findShortestPath(Vertex& s, Vertex& t, VertexList& path,ostream& out);
